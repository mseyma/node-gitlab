...
class Jobs extends BaseService {
  ...
  all(projectId: ProjectId, options?: PaginatedRequestOptions) {
    ...
    return RequestHelper.get(this, `${pId}/jobs`, options);
  }

  cancel(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.post(this, `${pId}/jobs/${jId}/cancel`, options);
  }
  ...
  downloadSingleArtifactFile(
    ...
    if (stream) {
      return RequestHelper.stream(
        this,
        `${pId}/jobs/${jId}/artifacts/${artifactPath}`,
        options,
      );
    }

    return RequestHelper.get(this, `${pId}/jobs/${jId}/artifacts/${artifactPath}`, options);
  }
  ...
  downloadLatestArtifactFile(
    ...
    if (stream) {
      return RequestHelper.stream(
        this,
        `${pId}/jobs/artifacts/${rId}/download?job=${jobName}`,
        options,
      );
    }

    return RequestHelper.get(this, `${pId}/jobs/artifacts/${rId}/download?job=${jobName}`, options);
  }

  downloadTraceFile(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.get(this, `${pId}/jobs/${jId}/trace`, options);
  }

  erase(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.post(this, `${pId}/jobs/${jId}/erase`, options);
  }

  keepArtifacts(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.post(this, `${pId}/jobs/${jId}/artifacts/keep`, options);
  }

  play(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.post(this, `${pId}/jobs/${jId}/play`, options);
  }

  retry(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.post(this, `${pId}/jobs/${jId}/retry`, options);
  }

  show(projectId: ProjectId, jobId: JobId, options?: Sudo) {
    const [pId, jId] = [projectId, jobId].map(encodeURIComponent);

    return RequestHelper.get(this, `${pId}/jobs/${jId}`, options);
  }
  ...
  showPipelineJobs(
    ...
    return RequestHelper.get(this, `${pId}/pipelines/${ppId}/jobs`, options);
  }
}
